  ...
  return (
    ...
    >
      <Link
        ...
        className="block relative pt-[100%] overflow-hidden bg-white"
      >
        <motion.img
        ...
        <div className="absolute bottom-3 left-3 flex gap-2 items-end">
          ...
          {!product.inStock && (
            <span className="bg-red-500 text-white px-2 py-1 text-xs rounded-full font-bold shadow-sm animate-pulse ml-2">
              Stokta Yok
            </span>
          )}
        </div>
        ...
        <div className="absolute top-3 right-3 flex flex-col gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
          ...
          >
            <GitCompare
              size={16}
              className={isInComparison(product.id) ? "fill-current" : ""}
            />
          </button>
        </div>
      </Link>
      ...
      <div className="p-5 flex-1 flex flex-col pt-6">
        <Link to={`/product/${product.id}`}>
          <h3
            className="text-gray-900 dark:text-white font-bold text-lg leading-tight hover:text-primary-600 dark:hover:text-primary-400 transition-colors line-clamp-2 mb-2"
            title={product.title}
          >
            {product.title}
          </h3>
